//
// NamedPipeImpl_WIN32.cpp
//
// $Id: //poco/1.4/Foundation/src/NamedPipeImpl_WIN32.cpp#2 $
//
// Library: Foundation
// Package: Processes
...
#include "Poco/NamedPipeImpl_WIN32.h"
	...
	_path(path.absolute())
{
	...
	if (_readHandle == INVALID_HANDLE_VALUE)
		throw CreateFileException("named pipe", GetLastError());
...
int NamedPipeImpl::writeBytes(const void* buffer, int length)
{
	...
	if (!ok)
		throw WriteFileException("named pipe", GetLastError());
...
int NamedPipeImpl::readBytes(void* buffer, int length)
{
	...
	if (ok || ((err = GetLastError()) == ERROR_BROKEN_PIPE) || (err == ERROR_PIPE_LISTENING) || (err == ERROR_NO_DATA))
		...
		throw ReadFileException("named pipe", GetLastError());
}
